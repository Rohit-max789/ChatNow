...
const Sidebar = ({ onSelectUser }) => {
  ...
  useEffect(() => {
    if (newMessageUsers && newMessageUsers.reciverId === authUser._id) {
      setUnreadMessages((prev) => ({
        ...prev,
        [newMessageUsers.senderId]: (prev[newMessageUsers.senderId] || 0) + 1,
      }));
    }
  }, [newMessageUsers, authUser._id]);
  ...
  useEffect(() => {
    const chatUserHandler = async () => {
      setLoading(true);
      try {
        const chatters = await axios.get(`/api/user/currchater`);
      ...
      {searchUser?.length > 0 ? (
        ...
        <>
          ...
          <div className="mt-auto px-1 py-1 flex">
            ...
            <p className="text-sm py-1" style={{ color: "white" }}>
              Logout
            </p>
          </div>
        </>
